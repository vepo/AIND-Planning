...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        current_state = self.s_levels[level]
        for action in self.all_actions:
            a_node = PgNode_a(action)
            if a_node.prenodes.issubset(current_state):
                for s_node in current_state:
    ...
    def add_literal_level(self, level):
        ...
        for action in self.a_levels[level - 1]:
            for state in action.effnodes:
                if state in self.s_levels[level]:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        return (
            len(set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)) > 0 or
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        return (
            len(set(node_a1.action.effect_add) & set(node_a2.action.precond_neg)) > 0 or
        ...
        for goal in self.problem.goal:
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    if goal == state.symbol and state.symbol not in self.fs.pos:
                        level_sum += level + 1
                        break
                else:
                    continue
